...
extern const uint8_t VESC_ID_MIN;
extern const uint8_t VESC_ID_MAX;
...
typedef struct {
  ...
  VescData data_[8];    // 受信データ（VESC ID 1-8）
  uint8_t max_id_;      // 管理する最大ID
...
void om_vesc_core_get_output (const VescCore* core, uint32_t* id, uint8_t out[8], uint8_t* len);
